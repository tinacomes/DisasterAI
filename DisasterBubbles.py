...
Original file is located at
    https://colab.research.google.com/drive/160Ecgu49VMixoy4fPSjXSL1YOtuVoU4Y
"""
...
class HumanAgent(Agent):
    ...
    def initialize_beliefs(self, assigned_rumor=None):
            ...
            # Initialize belief grid for ALL cells
            for x in range(width):
                for y in range(height):
                    cell = (x, y)
                    # Start with sensing actual disaster level in nearby cells
                    ...
                    if distance_from_agent <= sense_radius:
                        try:
                            ...
                            initial_conf = 0.7 if self.agent_type == "exploratory" else 0.5  # Explorers more confident in observations
                        except IndexError:
    ...
    def find_exploration_targets(self, num_targets=1):
        ...
        for cell, belief_info in self.beliefs.items():
            if isinstance(belief_info, dict):
                ...
                # Skip L0 cells entirely for primary exploration targets
                ...
                # --- NEW SCORING LOGIC: Weighted Sum ---
                ...
                # Add slight preference for cells further away? (Optional)
                ...
                # score += 0.05 * normalized_distance # Very small spatial push

                candidates.append({'cell': cell, 'score': score, 'level': level, 'conf': confidence})

        # --- Fallback remains: Lowest confidence overall if no L1+ candidates ---
        if not candidates:
            # print(f" > Agt {self.unique_id} ({self.agent_type}): No L{min_level_to_explore}+ targets found. Using fallback.")
            min_conf = 1.1
            lowest_conf_cells = []
            # ... (code to find lowest_conf_cells) ...
            if lowest_conf_cells:
                  fallback_cell = random.choice(lowest_conf_cells)
                  candidates = [{'cell': fallback_cell, 'score': -1, 'level': '?', 'conf': min_conf}] # Fallback score doesn't matter

        # Sort by score (highest score first)
        if candidates:
            candidates.sort(key=lambda x: x['score'], reverse=True)
            # Debug print top candidates:
            # if self.unique_id=="H_23" and self.model.tick % 10 == 1: # Debug specific agent periodically
            #     print(f"DEBUG Tick {self.model.tick} Agt {self.unique_id} Top Explore Candidates:")
            #     for cand in candidates[:min(5, len(candidates))]: print(f"  Cell:{cand.get('cell')} Lvl:{cand.get('level')} Conf:{cand.get('conf'):.2f} Score:{cand.get('score'):.3f}")
        ...
        try:
            ...
            if self.agent_type == "exploitative":
                ...
                if not interest_point or self.beliefs.get(interest_point, {}).get('level', 0) <= 0:
                    # Fallback: Ask most trusted friend or highest confidence cell
                    ...
                    if not interest_point or self.beliefs.get(interest_point, {}).get('level', 0) <= 0:
                        ...
                        interest_point = random.choice(highest_conf_cells) if highest_conf_cells else (random.randrange(self.model.width), random.randrange(self.model.height))
            ...
            # Source selection (epsilon-greedy)
            ...
            # More strategic source selection based on agent type
            if random.random() < self.epsilon:  # Exploration: random choice
                chosen_mode = random.choice(possible_modes)
            else:  # Exploitation: best known source with type-specific biases
                ...
                if self.agent_type == "exploitative":
                    ...
                    # High AI alignment makes exploitative agents more likely to consult AI
                    # (since AI will tend to confirm their beliefs)
                    ai_alignment_factor = self.model.ai_alignment_level * 0.2
                    for k in range(self.model.num_ai):
                        ai_id = f"A_{k}"
                        scores[ai_id] += ai_alignment_factor * self.trust.get(ai_id, 0.1)
                    ...
                    # (since AI will provide more accurate information)
                ...
                for mode in scores:
                    scores[mode] += random.uniform(-0.01, 0.01)
            ...
            if chosen_mode == "self_action":
                ...
                if not valid_sources:
                    return
                source_id = random.choice(valid_sources) if not self.friends else max(
                    [(fid, self.trust.get(fid, 0.1)) for fid in self.friends], key=lambda x: x[1])[0]
            ...
            for cell, reported_value in reports.items():
                ...
                old_level = belief_info.get('level', 0)
                reported_level = int(round(reported_value))
                d = abs(reported_level - old_level)

                P_accept_base = 1.0 if d == 0 else (self.D ** self.delta) / ((d ** self.delta) + (self.D ** self.delta))
                trust = self.trust.get(source_id, 0.1) if source_id else 0.1
                P_accept = P_accept_base + (trust - 0.5) * (0.5 if self.agent_type == "exploratory" else 0.3) + (0.2 if source_id and source_id.startswith("A_") else 0.0)
                P_accept = max(0.01, min(0.99, P_accept))

                if random.random() < P_accept:
                    new_level = max(0, min(5, int(round((belief_info.get('confidence', 0.1) * old_level + trust * reported_level) / (belief_info.get('confidence', 0.1) + trust)))))
                    new_confidence = min(0.99, belief_info.get('confidence', 0.1) + 0.01 + (trust - belief_info.get('confidence', 0.1)) * 0.3)
...
class DisasterModel(Model):
    ...
    def step(self):
        ...
        if self.tick % 5 == 0:
            ...
            # Filter agents who made calls AND had AI ratio > 0.5
            ai_reliant_agents = [agent for agent in self.humans.values()
                                if agent.accum_calls_total > 0 and (agent.accum_calls_ai / agent.accum_calls_total) > 0.5]

            aeci_variance = np.nan # Default to NaN if no data

            if ai_reliant_agents: # Only calculate if we have relevant agents
                ai_reliant_beliefs = []
                ...
                if ai_reliant_beliefs and global_var > 1e-9: # Need beliefs and non-zero global var
                    ai_reliant_var = np.var(ai_reliant_beliefs)
                    aeci_variance = max(0, min(1, (global_var - ai_reliant_var) / global_var)) # Removed epsilon adding
                elif ai_reliant_beliefs: # Have beliefs but global_var is zero
                    aeci_variance = 0.0 # If global var is 0, no reduction is possible
                # else: keep NaN if no beliefs collected

            # Append NaN if calculation wasn't possible, otherwise the value (could be 0)
            self.aeci_variance_data.append((self.tick, aeci_variance if not np.isnan(aeci_variance) else 0.0)) # Append 0 instead of NaN for simplicity downstream? Or handle NaN in plotting. Let's append 0.0 for now.
            # print(f"DEBUG Tick {self.tick}: Num AI reliant={len(ai_reliant_agents)}, AECI_Var={aeci_variance}") # DEBUG
            ...
            total_calls_expl = sum(agent.accum_calls_total for agent in self.humans.values() if agent.agent_type == "exploratory")
            ai_calls_expl = sum(agent.accum_calls_ai for agent in self.humans.values() if agent.agent_type == "exploratory")
            # print(f"Tick {self.tick}: Exp calls (AI/Total) = {ai_calls_exp}/{total_calls_exp}, Expl calls (AI/Total) = {ai_calls_expl}/{total_calls_expl}")
...
def simulation_generator(num_runs, base_params):
    ...
    def safe_stack(data_list):
         if not data_list or not any(isinstance(item, np.ndarray) and item.size > 0 for item in data_list):
             return np.array([]) # Return empty array if no valid data
         # Basic stacking, assumes consistent shapes from generator
         try:
             # Ensure all arrays have at least 2 dimensions for consistent stacking if necessary
             # This might need refinement depending on expected shapes (e.g., for 1D metrics)
             processed_list = []
             target_shape_len = 0
             for item in data_list:
                 if isinstance(item, np.ndarray) and item.size > 0:
                     if target_shape_len == 0:
                         target_shape_len = len(item.shape)
                     if len(item.shape) == target_shape_len:
                          processed_list.append(item)
                 # Handle or log inconsistent shapes/empty arrays if necessary
             if not processed_list: return np.array([])
             return np.stack(processed_list, axis=0)
         except ValueError as e:
             print(f"Warning: Stacking failed, possibly due to inconsistent shapes: {e}")
             return np.array([])
...
def plot_final_performance_vs_alignment(results_b, alignment_values, title_suffix="Exp B"):
    """Plots final mean performance metrics vs AI alignment."""
    num_params = len(alignment_values)
    bar_width = 0.2
    index = np.arange(num_params)
    ...
    final_correct_ratio_exploit, final_correct_ratio_explor = [], [] # Ratios don't have simple error bars here
    ...
    for align in alignment_values:
        ...
        def get_final_stats(data_array, col_index):
             if data_array is None or data_array.size == 0 or data_array.ndim < 2 or data_array.shape[1] == 0: return 0, [0, 0]
             final_vals = data_array[:, -1, col_index]
             mean = np.mean(final_vals); p25 = np.percentile(final_vals, 25); p75 = np.percentile(final_vals, 75)
             err = [max(0, mean - p25), max(0, p75 - mean)]
             return mean, err

        # Helper for unmet needs (list of lists)
        def get_final_unmet_stats(unmet_list):
            if not unmet_list: return 0, [0, 0]
            final_vals = []
            for run_data in unmet_list:
                if run_data: final_vals.append(run_data[-1])
            if not final_vals: return 0, [0, 0]
            mean = np.mean(final_vals); p25 = np.percentile(final_vals, 25); p75 = np.percentile(final_vals, 75)
            err = [max(0, mean - p25), max(0, p75 - mean)]
            return mean, err

        mean, err = get_final_stats(res.get("belief_error"), 1); final_mae_exploit_mean.append(mean); final_mae_exploit_err[0].append(err[0]); final_mae_exploit_err[1].append(err[1])
        mean, err = get_final_stats(res.get("belief_error"), 2); final_mae_explor_mean.append(mean); final_mae_explor_err[0].append(err[0]); final_mae_explor_err[1].append(err[1])
        mean, err = get_final_unmet_stats(res.get("unmet_needs_evol")); final_unmet_mean.append(mean); final_unmet_err[0].append(err[0]); final_unmet_err[1].append(err[1])
        final_correct_ratio_exploit.append(res.get("assist_ratio", {}).get("exploit_ratio", {}).get("mean", 0))
        final_correct_ratio_explor.append(res.get("assist_ratio", {}).get("explor_ratio", {}).get("mean", 0))

    fig, axes = plt.subplots(1, 3, figsize=(18, 5), sharex=True)
    fig.suptitle(f"Final Performance vs AI Alignment ({title_suffix})", fontsize=16)

    # Plot Final MAE
    axes[0].bar(index - bar_width/2, final_mae_exploit_mean, bar_width, yerr=np.array(final_mae_exploit_err), capsize=4, label='Exploit', color='red')
    axes[0].bar(index + bar_width/2, final_mae_explor_mean, bar_width, yerr=np.array(final_mae_explor_err), capsize=4, label='Explor', color='blue')
    axes[0].set_ylabel("Final Mean MAE")
    axes[0].set_title("Belief Error (MAE)")
    axes[0].legend()
    axes[0].grid(True, axis='y', linestyle='--', alpha=0.6)

    # Plot Final Unmet Needs
    axes[1].bar(index, final_unmet_mean, bar_width*1.5, yerr=np.array(final_unmet_err), capsize=4, label='Overall', color='purple')
    axes[1].set_ylabel("Final Mean Unmet Needs")
    axes[1].set_title("Unmet Needs")
    axes[1].set_xlabel("AI Alignment Level")
    axes[1].grid(True, axis='y', linestyle='--', alpha=0.6)

    # Plot Final Correct Token Share
    axes[2].bar(index - bar_width/2, final_correct_ratio_exploit, bar_width, label='Exploit', color='red')
    axes[2].bar(index + bar_width/2, final_correct_ratio_explor, bar_width, label='Explor', color='blue')
...
# ---  CONSOLIDATED PLOT 2: ECHO CHAMBERS ---
def plot_echo_chamber_indices(results_dict, title_suffix=""):
    ...
    event_ticks_list = results_dict.get("event_ticks_list", []) # List of lists
    # Use average event ticks or first run's? For simplicity, maybe just plot lines from first run
    avg_event_ticks = event_ticks_list[0] if event_ticks_list else []

    # Determine Ticks (use a reliable source like seci if possible)
    ticks = np.array([])
    if seci is not None and seci.ndim >=3 and seci.shape[1]>0:
        ticks = seci[0, :, 0] # Assumes tick is column 0
        if len(ticks) != seci.shape[1]: ticks = np.arange(seci.shape[1]) # Fallback
    if ticks.size == 0: print("Warning: Cannot determine ticks for echo plot."); return
    ...
    ax = axes[0, 0]
    _plot_mean_iqr(ax, ticks, seci, 1, "SECI Exploit", "maroon")
    _plot_mean_iqr(ax, ticks, seci, 2, "SECI Explor", "salmon")
    _plot_mean_iqr(ax, ticks, aeci, 1, "AI Call Ratio Exploit", "darkblue")
    _plot_mean_iqr(ax, ticks, aeci, 2, "AI Call Ratio Explor", "skyblue", linestyle='--')
    ...
    ax = axes[0, 1]
    _plot_mean_iqr(ax, ticks, retain_seci, 1, "Retain Friend (Exploit)", "green")
    _plot_mean_iqr(ax, ticks, retain_seci, 2, "Retain Friend (Explor)", "lightgreen", linestyle='--')
    _plot_mean_iqr(ax, ticks, retain_aeci, 1, "Retain AI (Exploit)", "purple")
    _plot_mean_iqr(ax, ticks, retain_aeci, 2, "Retain AI (Explor)", "plum", linestyle='--')
    add_event_lines(ax, avg_event_ticks)
    ...
    ax = axes[1, 0]
    _plot_mean_iqr(ax, ticks, comp_seci, 1, "Component SECI", "black")
    _plot_mean_iqr(ax, ticks, comp_aeci, 1, "Component AI Call Ratio", "grey")
    _plot_mean_iqr(ax, ticks, aeci_var, 1, "AI Bel. Var. Reduction", "magenta") # AI Echo Chamber
    add_event_lines(ax, avg_event_ticks)
    ...
    ax = axes[1, 1]
    _plot_mean_iqr(ax, ticks, comp_ai_trust_var, 1, "Component AI Trust Var.", "cyan")
    add_event_lines(ax, avg_event_ticks)
...
def plot_assistance_bars(assist_stats, raw_assist_counts, title_suffix=""):
    ...
    width = 0.35  # Increase spacing between groups
    
    fig, ax = plt.subplots(figsize=(10, 6))
    rects1 = ax.bar(x - width/2, mean_correct, width*0.8, yerr=yerr_correct, capsize=4, label='Mean Correct Tokens', color='forestgreen', error_kw=dict(alpha=0.5))
    rects2 = ax.bar(x + width/2, mean_incorrect, width*0.8, yerr=yerr_incorrect, capsize=4, label='Mean Incorrect Tokens', color='firebrick', error_kw=dict(alpha=0.5))
...
if __name__ == "__main__":
    ...
    if results_b: # Check if results exist before plotting summary
         ...
         # Add other summary plots as needed (e.g., final correct token ratio bar chart if not in performance plot)
