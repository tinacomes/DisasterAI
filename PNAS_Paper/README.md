...
**Fig. 1 has no image file** — it is the TikZ concept diagram drawn
inline in `DisasterAIFilter_PNAS.tex`, so it appears only once the
document is compiled. Nothing is missing from `Figures/`.

Main text: `fig2_configuration` (6 panels: per-type SECI vs α, per-type
chamber **evolution within runs**, per-type MAE, unmet needs),
`fig3_goldilocks` (3 panels: interior optimum, **unmet-needs
trajectories showing the speed-vs-coverage trade-off**,
...
`figS8_timeseries` (archived full time series, all α).

`allocation_diagnostics.csv` holds the per-α herding diagnostics
(aid concentration, aid–severity correlation, near-cell oversupply,
time to unmet-needs thresholds, residue) behind Fig. 3b and Table S11;
it is regenerated by `make_figures.py`.
