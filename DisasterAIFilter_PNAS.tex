...
%%
%% NUMBERS STATUS. All main-text numbers are now filled from
%% results-final/ (N=20, 200 ticks). The \fillnum macro is retained but
%% unused. Two claims were REVISED against the N=20 data and no longer
%% match the brief in PNAS_WRITING_INSTRUCTIONS section 3:
%%
%%  (1) The brief's "AECI-IE deepens with alpha while ~0 at the truthful
%%      endpoint" holds for CONFIRMATION-SEEKERS (-0.18 at alpha=0,
%%      seed-robustly deeper at every level 0.1-0.8, deepest -0.46 at
%%      0.5) but reverts at alpha>=0.9 where delivery saturates. For
%%      ACCURACY-SEEKERS the index is flat near zero in the main model;
%%      their harm runs through pool collapse, which a variance ratio
%%      over a shrinking pool cannot register. An earlier 4-seed/150-tick
%%      local zoom suggested the opposite assignment; it does not
%%      replicate at N=20/200 ticks and has been discarded.
%%  (2) INTERIORITY OF alpha*. Under the M1 primary composite
%%      |SECI|+|AECI-IE| the minimum is at alpha*=0 in the main model,
%%      NOT interior: removing AECI-Var's truth-convergence confound
%%      removes the artifact that produced the interior minimum. The
%%      interior optimum survives on RAW OPERATIONAL OUTCOMES instead
%%      (unmet needs, paired per-seed contrasts, both configurations),
%%      and Finding 2 is re-grounded on those. This is an author-level
%%      reframing of the Goldilocks claim - see RESULTS_OVERVIEW section 9.
%%
%% GAP SWEEP (Fig. 3b): run 32425907960, archived as
%% results-gap-sweep-fixed/. NOTE the archive is partial - GitHub's
%% artifact API paginates at 100, so 99 of 132 per-cell JSONs came
%% through; the missing 33 are all in the g=0.0 and g=0.5 rows. The
%% FIGURE is complete (the plot job built it from all 132 cells before
%% upload); only the raw cells are partial, so alpha* was recomputed
%% only for the fully resolved g=1.0 and g=1.5 rows. To complete the
%% archive, re-dispatch Archive Run Artifacts with
%% pattern={gap-cell-g0.0-*,gap-cell-g0.5-*} and the same dest.
%%
...
Whether AI can build or break community echo chambers depends first on the structure of information access --- the condition Gap~1 identified as untested. Reported per agent type, the two configurations diverge sharply at high alignment (Fig.~\ref{fig:comparison}a,b). Under unrestricted access, high alignment \emph{dissolves} the confirmation-seekers' echo chamber: SECI for exploitative communities rises from $-0.48$ at the truthful endpoint to $+0.01$ and $+0.03$ at $\alpha = 0.9$ and $1.0$ --- it crosses zero, so those communities end up marginally \emph{more} diverse than the population. Under network-gated access the same chamber persists, at $-0.28$ and $-0.27$. The paired per-seed difference is large and seed-robust: $\Delta \text{SECI}_{\text{exploit}} = -0.290$ $[-0.410, -0.170]$ and $-0.298$ $[-0.401, -0.196]$ (main $-$ control; Supplementary Table~S1).

The accuracy-seekers behave differently, and the contrast is instructive. Their communities deepen with alignment in \emph{both} configurations ($-0.01 \rightarrow -0.33$ main; $0.00 \rightarrow -0.29$ control), so the $\alpha$-gradient of any combined index is explorer-driven everywhere, while the configuration contrast is exploiter-driven. Reporting SECI per type is therefore not a refinement but a requirement: the combined index conceals two mechanisms that run on different axes.
...
The accuracy cost is borne almost entirely by the accuracy-seekers. Explorer belief error degrades monotonically with alignment, from $0.54$ at $\alpha = 0$ to $1.73$--$1.74$ at $\alpha \geq 0.9$ in the main model ($0.62 \rightarrow 1.94$--$1.96$ in the control), while exploiter error is flat at approximately $1.8$ across the entire sweep (Fig.~\ref{fig:comparison}c). Confirmation-seekers are already insulated by their narrow acceptance windows; there is little accuracy left to take from them. The mechanism on the explorer side is visible in the information they hold at all: their pool of beliefs about affected cells collapses from roughly 114 to 27 per agent between $\alpha = 0$ and $\alpha = 1$ (109 to 19 in the control). A confirming AI echoes their mostly-empty priors about the uncertain cells they deliberately query, so they stop acquiring disaster knowledge altogether. This is belief starvation, and it matters for interpretation: part of the explorer SECI deepening in Sec.~\ref{sec:claim1} is this pool shrinkage rather than convergence on a shared false narrative, which is why the pool is reported alongside SECI throughout (Supplementary Fig.~S2).

The AI-channel index tracks the same asymmetry, and at the confirming end it reproduces Finding~1 on the AI side. For confirmation-seekers, AECI-IE deepens from $-0.18$ at the truthful endpoint to $-0.46$ at $\alpha = 0.5$, lying seed-robustly below its own $\alpha = 0$ value at every level from $0.1$ to $0.8$ (paired within-configuration contrasts, 95\% CI excluding zero). Above that it reverts, to $-0.16$ at $\alpha = 1.0$, and AECI-IE-rel approaches zero ($-0.07$): once delivered confirmation saturates, the AI serves these communities precisely the beliefs they already hold, so it has ceased to be an independent source of information rather than becoming a narrower one. The configuration contrast at exactly that boundary mirrors Sec.~\ref{sec:claim1}: under unrestricted access the AI-channel bubble dissolves along with the social one ($-0.02$ and $-0.01$ at $\alpha = 0.9$ and $1.0$), while under network-gated access it persists ($-0.22$ and $-0.16$), a seed-robust paired difference of $-0.201$ $[-0.340, -0.063]$ and $-0.154$ $[-0.266, -0.042]$. The two channels through which beliefs are supplied therefore agree --- what the shared construct was built to test.

For accuracy-seekers the AI channel shows no comparable narrowing under network-bounded access: AECI-IE stays within $0.09$ of zero at every level. Their harm runs not through what a report conveys but through how few reports carry information at all, and a variance ratio over a collapsing pool cannot register that --- which is why the pool size, not the index, is the evidence for starvation. Where mobility provides no first-hand anchor the index does register it: in the control, explorer AECI-IE falls to $-0.38$ and $-0.35$ at $\alpha = 0.9$ and $1.0$ (paired differences $+0.312$ $[+0.211, +0.413]$ and $+0.296$ $[+0.196, +0.395]$).

A manipulation check bounds how the dose should be read. Because reports are delivered as integer severity levels, the fraction that exactly matches the caller's prior --- the \emph{delivered} confirmation --- is a step function of nominal $\alpha$: $0.14$ at $\alpha = 0.3$, $0.60$ at $\alpha = 0.5$, $0.93$ at $\alpha = 0.8$, and exactly $1.00$ at $\alpha \geq 0.9$, where $\alpha = 0.9$ and $\alpha = 1.0$ are identical in what they serve. The operational transition between $\alpha = 0.8$ and $\alpha = 0.9$ is therefore a saturation boundary of the delivery mechanism, not a threshold in the policy (Supplementary Table~S2).

The alignment optimum is interior, but the evidence for it is operational rather than index-based. Relief delivery is seed-robustly better at intermediate alignment than at perfect truthfulness: in the main model, unmet high-need cells fall from $1.11$ per tick at $\alpha = 0$ to $0.38$--$0.67$ for $\alpha \in [0.4, 0.7]$ (paired within-configuration contrasts $-0.44$ to $-0.73$, 95\% CIs excluding zero), then rise seed-robustly above the truthful endpoint once confirmation saturates ($+1.61$ and $+1.97$ at $\alpha = 0.9$ and $1.0$). The control traces the same shape more sharply ($2.17 \rightarrow 0.49$--$1.23$, then $9.94$ and $10.53$). A modest amount of confirmation buys adoption --- agents accept and act on more of what they are told --- and that gain outweighs the distortion until delivery saturates. These are raw paired per-seed contrasts on a decision outcome, so the result rests on no normalization and no choice of echo index (Fig.~\ref{fig:comparison}d).

The composite indices tell a more equivocal story, and the metric revision is why. The retired variance-over-users index placed $\alpha^{*}$ in the interior --- but it is \emph{most} negative at the truthful endpoint, where AI-reliant agents are homogeneous because they are all approximately correct, so the interior minimum it produced was partly a truth-convergence artifact. With AECI-IE, which carries no such confound, the primary composite is minimized at $\alpha^{*} = 0$ in the main model, while composites on SECI alone, the channel baseline, or the error contrast place it between $0.2$ and $0.5$ (Fig.~\ref{fig:goldilocks}a; Supplementary Table~S2). Interiority is thus not a property of the echo indices but of the decision outcomes they are meant to anticipate.

The same split appears across the population's cognitive profile. In the cognitive-gap sweep, the alignment level that minimizes unmet needs is interior in every fully resolved cell of the grid --- between $0.5$ and $0.7$ --- whether the two cognitive types differ moderately or strongly, and whether the population is closed, baseline or open in what it will accept (Fig.~\ref{fig:goldilocks}b). The index-based optimum is not stable in the same way: over those same cells the primary bubble composite selects the truthful endpoint in half and an interior level in the rest, while the retired variance-over-users composite selects an interior level everywhere --- the same contrast as in the primary sweep, and for the same reason. The cost paid \emph{at} the optimum rises with both polarization and closedness: populations needing more confirmation to accept information are worse off even under their own best policy.
...
Confirmation-seekers are captured. Their share of queries directed to the AI rises monotonically with alignment, from $0.56$ to $0.70$, and their trust in it edges up alongside ($0.47 \rightarrow 0.49$): the source that satisfies their acceptance window is selected more often, which exposes them to more of it. Accuracy-seekers show the complementary failure. They do \emph{not} learn to distrust a confirming AI --- their trust in it stays high across the entire sweep ($0.88 \rightarrow 0.84$ at $\alpha = 1$) --- because verification is base-rate dominated: approximately $90\%$ of queried cells are truly empty, so an AI that echoes mostly-correct `no impact' priors passes external verification on almost everything it says, failing only on the rare high-severity cells that matter. Their beliefs freeze instead: relative to their AI-light peers, AI-heavy explorers' belief mobility declines from $-0.01$ to $-0.14$ with alignment (Supplementary Fig.~S3). The failure of accuracy-seeking under base-rate-dominated verification is a substantive finding, not a design limitation; a salience-weighted verification channel, in which errors about disaster cells outweigh confirmations of empty ones, is retained as the counterfactual (Supplementary Fig.~S6).

Structure buffers the operational consequences even as it preserves the epistemic ones. At $\alpha \geq 0.9$, unmet high-need cells reach $9.9$--$10.5$ per tick in the control but only $2.7$--$3.1$ in the main model; exploiter targeting precision is $0.24$--$0.25$ against $0.36$--$0.38$, and explorer precision collapses to $0.20$ in the control against $0.60$--$0.63$ in the main model. Two mechanisms produce this, both independent of the AI: mobility gives first-hand observation coverage that no confirmation can distort, and network-gated querying concentrates exchange on contacts whose local observations are accurate about the caller's surroundings. Because part of every reward signal stays grounded in observation, source-selection learning keeps receiving corrective evidence.
...
The fourth finding traces how harm is distributed --- the equity question of Gap~4. In the main model, the belief-error gap between agents spawned in the furthest and nearest quartiles from the epicenter widens monotonically with alignment, from $+0.15$ at $\alpha = 0$ to $+0.30$ at $\alpha = 1$ (Fig.~\ref{fig:periphery}a): agents remote from events, who depend almost entirely on mediated reports, absorb the accuracy loss confirmation produces. The behavioral consequence is withdrawal. Relief contributed by far-spawned agents falls from $16.5$ to $6.2$ tokens per agent per five-tick window across the sweep, a decline of $62\%$, against $18.3$ to $12.6$ ($-31\%$) for near-spawned agents (Fig.~\ref{fig:periphery}b) --- the remote quartile does not merely become less accurate, it largely stops sending assistance.

Network position offers no comparable protection: belief-error differences between high- and low-betweenness agents stay within $0.07$ at every alignment level (Supplementary Table~S3). In the immobile control both gaps are approximately zero at all $\alpha$, as the structural-null design requires. The harms of a confirming AI are thus spatially, not topologically, structured: they fall on those who cannot check reports against first-hand observation, however well connected they are --- precisely the position of the remote coordination hubs that dominate data-driven crisis response \citep{comes2020coordination,coleman2024weaving}.
